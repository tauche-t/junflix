...
import { getPopular, getTopRated, IGetMoviesResult } from "../../api";
...
function TopReated() {
  ...
  const bigMovieMatch = useRouteMatch<{movieId: string}>("/movies/topReated/:movieId");
  const { scrollY } = useViewportScroll();
  const [left, setLeft] = useState(false);
  const { data , isLoading } = useQuery<IGetMoviesResult>(["topReatedMovies", "topReated"], getTopRated);
  ...
  const onBoxClicked = (movieId: number) => {
    history.push(`/movies/topReated/${movieId}`);
  }
  ...
  const clickedMovie = bigMovieMatch?.params.movieId + "topReated" && data?.results.find(movie => movie.id+"topReated" === bigMovieMatch?.params.movieId + "topReated");

  return (
      <>
        <Slider>
          <h3 style={{ paddingLeft: "20px" }}>TopReated</h3>
        ...
        {bigMovieMatch ?
          <AnimatePresence>
              <>
                ...
                <BigMovie layoutId={bigMovieMatch.params.movieId + "topReated"} style={{ top: scrollY.get() + 100 }}>
                  {clickedMovie && (
                    <>
