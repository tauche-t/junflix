...
import { getAirTv, IGetTvResult } from "../../api";
...
function Air() {
  ...
  const bigMovieMatch = useRouteMatch<{tvId: string}>("/tv/air/:tvId");
  const { scrollY } = useViewportScroll();
  const [left, setLeft] = useState(false);
  const { data , isLoading } = useQuery<IGetTvResult>(["airTv", "on_the_air"], getAirTv);
  ...
  const clickedMovie = bigMovieMatch?.params.tvId + "air" && data?.results.find(movie => movie.id + "air" === bigMovieMatch?.params.tvId + "air");

  return (
    <>
        <Slider>
          <h3 style={{ paddingLeft: "20px" }}>On The Air</h3>
        ...
        {bigMovieMatch ?
          <AnimatePresence>
              <>
                ...
                <BigMovie layoutId={bigMovieMatch.params.tvId + "air"} style={{ top: scrollY.get() + 100 }}>
                  {clickedMovie && (
                    <>
