...
import { getAirTv, getPopularTv, getTopRatedTv, IGetTvResult } from "../../api";
...
function Popular() {
  ...
  const bigMovieMatch = useRouteMatch<{tvId: string}>("/tv/topRated/:tvId");
  const { scrollY } = useViewportScroll();
  const [left, setLeft] = useState(false);
  const { data , isLoading } = useQuery<IGetTvResult>(["topRatedTv", "top_rated"], getTopRatedTv);
  ...
  const clickedMovie = bigMovieMatch?.params.tvId + "topRated" && data?.results.find(movie => movie.id + "topRated" === bigMovieMatch?.params.tvId + "topRated");

  return (
    <>
        <Slider>
          <h3 style={{ paddingLeft: "20px" }}>TopRated</h3>
        ...
        {bigMovieMatch ?
          <AnimatePresence>
              <>
                ...
                <BigMovie layoutId={bigMovieMatch.params.tvId + "topRated"} style={{ top: scrollY.get() + 100 }}>
                  {clickedMovie && (
                    <>
